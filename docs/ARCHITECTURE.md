...
### Текущее состояние (шаг 7)

Яндекс.Метрика подключена через `components/site/metrika.tsx` с `strategy="afterInteractive"`.  
Событие `lead_submit` отправляется через `lib/analytics.ts → trackGoal('lead_submit')` при успешной отправке формы.

Переменные окружения:
- `NEXT_PUBLIC_METRIKA_ID` — ID счётчика Яндекс.Метрика (строка, только цифры).

### Отложено до шага 12 (cookie-баннер)

Скрипт Метрики в текущей реализации инициализируется без явного согласия пользователя.  
Это **временное состояние**: по 152-ФЗ и GDPR персональные данные (включая идентификаторы визитов) нельзя собирать без согласия.

**Что нужно сделать в шаге 12:**

1. Реализовать cookie-баннер с принятием/отклонением.
2. При принятии вызывать `onConsentGranted()`, который:
   - Инициализирует `ym(id, 'init', { ... })`.
   - Сохраняет флаг согласия в `localStorage` / cookie.
3. Перенести вызов `ym(..., 'init', ...)` из `metrika.tsx` внутрь `onConsentGranted`.
4. `trackGoal` должен вызываться только если согласие уже получено — добавить guard в `lib/analytics.ts`.

TODO-комментарии уже расставлены в `components/site/metrika.tsx` и `lib/analytics.ts`.
